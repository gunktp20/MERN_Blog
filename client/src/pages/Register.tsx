...
function Register() {
    ...
    const onSubmit = async () => {
        ...
        if (password !== confirm_password) {
            setShowAlert(true);
            setAlertType("danger")
            setAlertText("กรุณากรอกรหัสผ่านเเละยืนยันรหัสผ่านให้ตรงกัน !")
            setTimeout(() => {
                setShowAlert(false);
                setAlertType("danger")
                setAlertText("")
        ...
        setAlertText("สมัครสมาชิกเสร็จสิ้นกรอกรหัสผ่านเพื่อเข้าสู่ระบบ")
        setTimeout(() => {
            setShowAlert(false);
            setAlertType("danger")
            setAlertText("")
    ...
    return (
        <Wrapper>
            <div className="form-main w-[400px] ">
                ...
                <div className="text-[12.5px] text-gray-600">กรอกข้อมูลของคุณเพื่อสมัครสมาชิก</div>
                ...
                <div className="relative z-0 w-full mb-5 group mt-7">
                    ...
                    }} type="password" name="username" id="username" className="block py-2.5 px-0 w-full text-[14.5px] text-gray-900 bg-transparent border-0 border-b-[1.6px] border-gray-200 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-[#2cb1bc] focus:outline-none focus:ring-0 focus:border-[#2cb1bc] peer" placeholder=" " required />
                ...
                <div className="relative z-0 w-full mb-5 group mt-7">
                    <input
                        ...
                        type="password" name="username" id="username" className="block py-2.5 px-0 w-full text-[14.5px] text-gray-900 bg-transparent border-0 border-b-[1.6px] border-gray-200 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-[#2cb1bc] focus:outline-none focus:ring-0 focus:border-[#2cb1bc] peer" placeholder=" " required />
                    <label htmlFor="username" className="peer-focus:font-medium absolute text-[12.5px] text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-[#2cb1bc] peer-focus:dark:text-[#2cb1bc] peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">confirm password</label>
                </div>
                <button onClick={onSubmit} disabled={isLoading} className="w-full bg-[#2cb1bc] text-white h-[38px] rounded-md mt-5 shadow-md">
                    Login
                </button>
                <div className="flex mt-2 justify-end pr-2">
                    <p className="text-[12.4px] text-[#333]">หากคุณเป็นสมาชิกอยู่เเล้ว</p>
                    <Link className="text-[12.4px] ml-2 text-[#333]" to="/login">
                        Login
                    </Link>
                </div>
            </div>
